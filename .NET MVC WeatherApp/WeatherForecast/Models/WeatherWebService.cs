      ...
      public List<Location> FindLocation(string city)
        {
            ...
            var requestUriString = String.Format(@"http://api.geonames.org/search?name_equals={0}&username=devHaris&featureClass=P&style=full", city);
            var document = LoadDocument(requestUriString);

            // Return the one and only found user.
            return document.Descendants("geoname")
                .Select(LocationFactory.Create)
                .ToList();
        }
        ...
        public List<Weather> FindWeather(Location location)
        {
            ...
            var requestUriString = String.Format(@"http://www.yr.no/place/{0}/{1}/{2}/forecast.xml", location.Country, location.County, location.City);
            var document = LoadDocument(requestUriString);

          var singleOrDefault = (from nu in document.Descendants("nextupdate") select nu).SingleOrDefault();
          if (singleOrDefault != null)
            NextUpdate = DateTime.Parse(singleOrDefault.Value);
